...
/**
 * Creates folders from passed path
 *
 * @param {string} dirPath
 */
module.exports.mkDir = (dirPath) => {
  const keys = path.parse(dirPath).dir.split(path.sep);

  if (keys[0] === '') {
    keys.shift();
    keys[0] = `${path.sep}${keys[0]}`;
  }

  let construcPath = '';
  while (keys.length > 0) {
    construcPath = path.join(construcPath, keys.shift());
    const dir = path.resolve(construcPath);

    if (!fs.existsSync(dir)) {
      try {
        fs.mkdirSync(dir);
      } catch (error) {
        console.error(`Cannot create dir ${dir}, ERROR: ${error.message}`);
        throw error;
      }
    }
  }
